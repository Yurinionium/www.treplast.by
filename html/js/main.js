$(document).ready(function () {
    ...
    if ($(".owl-carousel-main-slider").length > 0) {
        $(".owl-carousel-main-slider").owlCarousel({
            ...
            autoplay: true,
            autoHeight:false,
            autoplayTimeout: scrollDelayMain,
            navText: ['', ''],
            responsive: {
                0: {
                    nav: false
    ...
    $(document).on("click", ".reset_filters", function (e) {
        $(this).find('.reset_filters_counter').text('');

        var filters = $('.model_list table .filter-wrap ');

        filters.each(function () {
            $(this).closest('td').removeClass('filter-on active');
    ...
    function sendInfoAboutSubscriber(email) {
        $.ajax({
            url: "ajax.php",
            data: "action=sendSubscriberInfo&email=" + email,
            type: "POST",
            cache: false,
            dataType: 'json',
            beforeSend: function () {
    ...
    $("form.submitter").on("submit", function (e) {
        ...
        $.ajax({
            ...
            success: function (msg) {
                ...
                $("#" + a.data("target") + "_btn").trigger("click");
                var action = msg.action || '';
                var email = msg.email || '';
                if(action != '' && email != '') {
                    sendInfoAboutSubscriber(email);
                }
            }
        })
    });
            ...
            search.each(function () {
                ...
                openSearchBtn.off('click.search').on('click.search', function () {
                    if(window.innerWidth < 990) {
                        return window.location.href = $('.search-form').attr('action');
                    }
